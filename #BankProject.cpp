...
enum enOperation {nothing,clientsList,addClients,deleteClient,updateClient,findClient,logout};
...
string GetStringInSize (const string &message,unsigned short from,unsigned short to,const string &errormessage){
    string text ;
    unsigned size ;
    cout << message;
    do{
...
void OpenBankSystem (){
 enOperation operationNumber;
   do{  
        system("cls");
        PrintMainMenu ();
        operationNumber = (enOperation)PositiveShortNumberInRange("Choose what do you want to do? [1 to 6]?\n",1,6);
